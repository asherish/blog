...
上記のように、インストール方法には distribution-specific package（deb/rpm パッケージ）と distribution-independent package（runfile）の2種類があります。ガイドでも前者が推奨されています。

なお、runfile にも `--driver` フラグと `--toolkit` フラグの記載があり、ドライバーのみ、またはツールキットのみをインストールできるかもしれません。しかし、本記事ではガイドの推奨に従い distribution-specific package（deb）を採用します。
...
ここまで、CUDA のインストールガイドや開発者サイトを通じて NVIDIA のリポジトリを設定し、`cuda-drivers` パッケージでドライバーをインストールする方法を紹介しました。一方、Ubuntu では `ubuntu-drivers` コマンドを使って、Ubuntu のリポジトリから NVIDIA ドライバーをインストールすることもできます。

両者の違いは以下の通りです。

- **NVIDIA リポジトリ + `cuda-drivers`**: NVIDIA が提供するリポジトリからインストールする方法。最新のドライバーが提供される。将来 CUDA をインストールする場合にもリポジトリの設定をそのまま利用できる。
...
2. パッケージリストを最新の状態に更新してください。
   ...
   sudo apt upgrade -y
   ```
3. 「[インストールすべきバージョンを確認する](#インストールすべきバージョンを確認する)」で確認したバージョンのドライバーをインストールしてください（たとえば、バージョン `535` の場合）。
   ```bash
   sudo apt install -y nvidia-driver-535
   ```
:::message
`ubuntu-drivers devices` コマンドでも対応ドライバーの一覧と推奨バージョンを確認できますが、`recommended` と表示されたバージョンでも正しく動作しないことがあります。インストールすべきバージョンは、本記事の「[インストールすべきバージョンを確認する](#インストールすべきバージョンを確認する)」で紹介した [NVIDIA 公式ドライバーページ](https://www.nvidia.com/ja-jp/drivers/) で確認してください。GPU の型番は `sudo lshw -C display` で確認できます。
:::
