...
## Background on Installation Methods
...
Note that the runfile also has `--driver` and `--toolkit` flags documented, which may allow installing only the driver or only the toolkit separately. However, this article follows the guide's recommendation and uses distribution-specific packages (deb).
...
```bash
# Installing cuda-toolkit will install CUDA
# sudo apt-get -y install cuda-toolkit-<version>

# Installing cuda-drivers will install only the NVIDIA driver without CUDA
sudo apt-get -y install cuda-drivers
```

## How to Install the NVIDIA Driver Without CUDA

So far, we've introduced how to set up the NVIDIA repository through the CUDA Installation Guide and developer site, and install the driver using the `cuda-drivers` package. Alternatively, on Ubuntu, you can also install the NVIDIA driver from Ubuntu's repository using the `ubuntu-drivers` command.

Here are the differences between the two approaches:

- **NVIDIA repository + `cuda-drivers`**: Installs from NVIDIA's repository. Provides the latest drivers. The repository configuration can be reused if you install CUDA in the future.
...
2. Update the package list to the latest state.
   ...
   sudo apt upgrade -y
   ```
3. Install the driver version you confirmed in "[Check the Version to Install](#check-the-version-to-install)" (for example, version `535`).
   ```bash
   sudo apt install -y nvidia-driver-535
   ```
> ℹ️ You can also check available drivers and the recommended version using the `ubuntu-drivers devices` command, but the version marked as `recommended` may not always work correctly. Check the version to install on the [NVIDIA official driver page](https://www.nvidia.com/drivers/) as described in "[Check the Version to Install](#check-the-version-to-install)" above. You can find your GPU model with `sudo lshw -C display`.
